...
Think of it like moving a bubble up through water. Instead of swapping the bubble with each water molecule it passes, we just let the water flow down while the bubble rises. The bubble only settles into its final position once.

Here's the original version with swaps:

{% capture code %}Heap-Increase-Key(A, i, key)
    if key < A[i]
    ...
    while i > 1 and A[Parent(i)] < A[i]
        ...
        i = Parent(i)
{% endcapture %}
{% include clrs_code.html title="Original Heap-Increase-Key" %}

The exchange operation on line 5 requires three assignments:
...
{% capture code %}Heap-Increase-Key(A, i, key)
    ...
    A[i] = key
{% endcapture %}
{% include clrs_code.html title="Optimized Heap-Increase-Key" %}
...
{% capture note %}
This optimization mirrors the inner loop of $$\textsc{Insertion-Sort}$$, which also shifts elements rather than swapping them. The technique is effective because we know the final value (the key) from the start, so we don't need to carry it along during each iteration. We can simply remember it and place it once at the end.
{% endcapture %}
{% include aside.html title='Why This Works Better' %}
