...
Let's say $$\textsc{Tree-Insert}$$ examines nodes $$x_1, x_2, \ldots, x_k, \textsc{Nil}$$ where:
- $$x_1$$ is the root
...
3. Finds $$v$$ at the node that was inserted (let's call it $$x_{k+1}$$)

So $$\textsc{Tree-Search}$$ examines $$k + 1$$ nodes: $$x_1, x_2, \ldots, x_k, x_{k+1}$$.

### Why the paths are the same

The key insight is that **inserting $$v$$ doesn't change the search path to any value that's already in the tree**, and **searching for $$v$$ follows the same comparisons that were made during insertion**.
...
Wait, let me reconsider. The problem says "one plus the number of nodes examined when the value was first inserted." Let's recount:
