...
**Answer:** No, this array is not a max-heap. The max-heap property is violated at node 4, where the parent value (6) is less than its right child (7).

{% capture note %}
The violation at node 4 breaks the fundamental requirement of a max-heap. If we wanted to fix this array and make it a valid max-heap, we would need to apply $$\textsc{Max-Heapify}$$ at node 4 (and possibly other nodes), which would swap the 6 and 7, and then continue fixing any violations that result.
{% endcapture %}
{% include aside.html title='How to Fix This Array' %}
