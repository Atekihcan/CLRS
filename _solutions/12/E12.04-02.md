...
Here's why: If we have a path of length $$k$$ hanging off a balanced tree, the path contributes $$\Theta(k^2)$$ to the total depth sum (nodes at depths $$d, d+1, \ldots, d+k$$ contribute $$\sum_{i=0}^{k} i = \Theta(k^2)$$).
...
3. The path nodes don't dominate the average because there are few of them
