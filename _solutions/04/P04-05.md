...
The four possible outcomes of a test are:
...
### Part (a): If at least $$n/2$$ chips are bad, can we determine which are good?

**No.** If at least $$n/2$$ chips are bad, they can conspire to fool the professor.
...
Since $$|\text{bad}| \ge |\text{good}|$$, these scenarios cannot be distinguished through testing alone.

### Part (b): Find a single good chip assuming more than $$n/2$$ are good

**Algorithm:** Use $$\lfloor n/2 \rfloor$$ pairwise tests to reduce the problem size.

**FIND-GOOD-CHIP(S):**
1. If $$|S| = 1$$, return the chip in $$S$$
2. Pair up chips arbitrarily, leaving one unpaired if $$|S|$$ is odd
3. Test each pair:
...
### Part (c): Identify all good chips with $$\Theta(n)$$ pairwise tests
