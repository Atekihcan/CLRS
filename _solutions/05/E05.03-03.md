...
>
> {%- include clrs_code.html title="Permute-With-All(A)" -%}
>
...
Since $$4.5$$ is not an integer, it is impossible for each permutation to occur with equal probability. Some permutations must be more likely than others.
...
For $$n = 2$$, the distribution happens to be uniform ($$1/2 = 1/2!$$). However, this is a special case that does not generalize to larger $$n$$.
...
{% capture note %}
This exercise demonstrates that more randomness does not necessarily mean better randomness. The key is structured randomness that matches the combinatorial structure of the problem. The original algorithm's decreasing selection range precisely matches the constraints of building a permutation one element at a time.
{% endcapture %}
