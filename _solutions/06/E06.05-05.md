...
{% capture note %}
This loop invariant proof demonstrates that $$\textsc{Heap-Increase-Key}$$ correctly maintains the max-heap property by bubbling the increased element upward until it finds its proper position. The running time is $$O(\lg n)$$ because each iteration moves one level up the tree, and the tree has height $$O(\lg n)$$.
{% endcapture %}
