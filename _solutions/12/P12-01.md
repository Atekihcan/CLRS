...
- **Strategy C (lists)** is best if you expect many duplicate keys and primarily perform insertions. It is the fastest ($$O(n)$$) and simplest.
...
In practice, many BST implementations simply do not allow duplicate keys, or they use Strategy C (storing duplicates in a list/count at each node).
{% endcapture %}
