...
{% capture note %}
This bound is tight in the sense that you can construct heaps where exactly $$\lceil n/2^{h+1} \rceil$$ nodes have height $$h$$. For example, a complete binary tree with $$n = 2^{k+1} - 1$$ nodes has exactly $$2^{k-h}$$ nodes at height $$h$$ for $$h \leq k$$, which equals $$n/2^{h+1}$$ when $$n = 2^{k+1} - 1$$.
{% endcapture %}
