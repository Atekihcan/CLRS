...
2. If $$x$$ has no left subtree, the predecessor must be an ancestor. Specifically, it's the first ancestor for which $$x$$ is in the right subtree (since all keys in a right subtree are larger than the ancestor)
