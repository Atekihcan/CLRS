...
>
> {%- include clrs_code.html title="Permute-Without-Identity(A)" -%}
>
...
2. If it is the identity permutation, generate another one (expected number of trials: $$n! / (n! - 1) \approx 1.0003$$ for large $$n$$)
...
{% capture note %}
This exercise illustrates an important principle: making every element move does not guarantee a uniform distribution. The key to uniform random permutations is ensuring that each position can receive any element with appropriate probability, as in $$\textsc{Randomize-In-Place}$$.
{% endcapture %}
