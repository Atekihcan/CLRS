...
{% capture note %}
There is a subtle issue: if multiple elements have the same distance $$d$$ from the median, we might collect more than $$k$$ elements in step 4. To handle this precisely, after partitioning, we take all elements with distance strictly less than $$d$$, then add elements with distance exactly $$d$$ until we have exactly $$k$$ elements total. This does not change the asymptotic complexity.
{% endcapture %}
