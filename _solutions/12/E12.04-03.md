...
These two notions of randomness are fundamentally different. Let us examine the case $$n = 3$$ with keys $$\{1, 2, 3\}$$.

### Randomly Chosen BST (Each Tree Equally Likely)

First, let us enumerate all possible BST shapes with 3 nodes:
...
Let us trace through each insertion order:
...
The randomly chosen model is mathematically simpler (uniform distribution over trees) but does not correspond to any natural process of tree construction. It is mainly used for theoretical analysis in combinatorics.
{% endcapture %}
