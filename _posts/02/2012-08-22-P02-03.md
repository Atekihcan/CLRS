...
y' & = a_i + x \cdot y \\
   ...
   & = \textcolor{blue} {a_i x^0} + \sum_{k = 0}^{n - (i + 1)} a_{k + i + 1}x^{k + 1} \\
   & = \textcolor{blue} {a_{-1 + i + 1} x^{-1 + 1}} + \sum_{k = 0}^{n - (i + 1)} a_{k + i + 1}x^{k + 1}\\
...
{% capture note %}
Note the first terms of line 3 and 4  in the above derivation (marked in blue). In mathematical derivations, it is often very common to do such manipulations to arrive at the desired conclusion. But how do you know what assumption you will need to make?

The goal here was to show that $$y'$$ can be expressed as the summation mentioned in problem statement. To achieve that we are simply rewriting the $$a_i$$ term to have a format like $$a_{k+i+1} x^{k+1}$$ so that we can consume the term in our summation. And this is done with assuming $$k = -1$$.
{% endcapture %}
