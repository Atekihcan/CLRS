...
{% capture code %}Heap-Decrease-Key(A, i, key)
    if key > A[i]
    ...
    while i > 1 and A[Parent(i)] > A[i]
        ...
        i = Parent(i)
{% endcapture %}
{% include clrs_code.html title="Heap-Decrease-Key" %}
...
{% capture code %}Min-Heap-Insert(A, key)
    A.heap-size = A.heap-size + 1
    A[A.heap-size] = ∞
...
{% capture note %}
These procedures form a complete min-priority queue implementation. They support the core operations ($$\textsc{Insert}$$, $$\textsc{Minimum}$$, $$\textsc{Extract-Min}$$, $$\textsc{Decrease-Key}$$) needed for many algorithms, including Dijkstra's shortest path algorithm (Chapter 24) and Prim's minimum spanning tree algorithm (Chapter 23).
{% endcapture %}
{% include aside.html title='Min-Priority Queue Applications' %}
