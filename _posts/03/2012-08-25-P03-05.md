...
{% capture note %}
The difference between regular $$\Omega$$ and $$\mathop \Omega \limits^\infty$$, is that in the regular case the definition involves _all $$n$$ greater than a large enough $$n_0$$_, whereas in the other case it is for _infinitely many $$n$$_. Although the former includes the later, the opposite is not true as evident from the counter-example shown above.
