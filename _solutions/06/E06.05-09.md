...
{% capture code %}Merge-K-Lists(lists, k)
    // lists is an array of k sorted lists
    // Each element in heap stores (value, list_index, element_index)
    ...
    for i = 1 to k
        if lists[i] is not empty
            Min-Heap-Insert(H, (lists[i][1], i, 1))

    // Extract minimum and refill from same list
    for j = 1 to n
        if H is empty
            error "lists contained fewer than n elements"
        (value, list_index, elem_index) = Heap-Extract-Min(H)
        result[j] = value
        next_index = elem_index + 1
        if next_index ≤ lists[list_index].length
            Min-Heap-Insert(H, (lists[list_index][next_index], list_index, next_index))

    return result
{% endcapture %}
{% include clrs_code.html title="Merge-K-Lists" %}
