...
description: "We can build a heap by repeatedly calling Max-Heap-Insert to insert the elements into the heap. Consider the following variation on the Build-Max-Heap procedure: Do the procedures Build-Max-Heap and Build-Max-Heap' always create the same heap when run on the same input array? Prove that they do, or provide a counterexample. Show that in the worst case, Build-Max-Heap}' requires Theta (n \lg n) time to build an n-element heap."
---
...
>We can build a heap by repeatedly calling $$\textsc{Max-Heap-Insert}$$ to insert the elements into the heap. Consider the following variation on the $$\textsc{Build-Max-Heap}$$
procedure:
>
>{%- include clrs_code.html title="Build-Max-Heap`(A)" -%}
>
...
![Build-Max-Heap Result]({{ '/assets/img/06/6-1_build_max_heap.png' | prepend: site.baseurl }} "Result using Build-Max-Heap")
...
![Build-Max-Heap' Result]({{ '/assets/img/06/6-1_build_max_heap_prime.png' | prepend: site.baseurl }} "Result using Build-Max-Heap'")
