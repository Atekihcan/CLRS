...
The space savings can be substantial. For example, on a 64-bit machine where pointers are 8 bytes (64 bits), a standard direct-address table for $$m = 1000$$ keys would require 8000 bytes, while a bit vector requires only 125 bytes (1000 bits ÷ 8 bits/byte). This is a 64-fold reduction in space.
