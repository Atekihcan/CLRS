...
Think of it like removing a person from a waiting line. We fill their spot with the last person in line, then either move that person forward (if they have higher priority than those around them) or backward (if they have lower priority).
...
{% capture code %}Heap-Delete(A, i)
    ...
    if i > 1 and A[i] > A[Parent(i)]
        ...
        Max-Heapify(A, i)
{% endcapture %}
{% include clrs_code.html title="Heap-Delete" %}

{% include ads.html %}

The procedure works as follows:

**Step 1:** Check if the index is valid. If $$i$$ is the last element, simply decrease the heap size and return.

**Step 2:** Replace $$A[i]$$ with the last element $$A[\text{heap-size}]$$ and decrease the heap size.

**Step 3:** Determine which direction to restore the heap property:
   - If $$A[i]$$ is now larger than its parent (line 8), call $$\textsc{Heap-Increase-Key}$$ to bubble it upward
   - Otherwise, call $$\textsc{Max-Heapify}$$ to let it float downward
...
{% capture note %}
An alternative implementation always calls $$\textsc{Heap-Increase-Key}(A, i, \infty)$$ to bubble the element to the root, then calls $$\textsc{Heap-Extract-Max}$$. This is simpler but makes two passes through the heap instead of one. Our implementation is more efficient in practice.
{% endcapture %}
