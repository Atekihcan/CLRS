...
Wait, that's not quite right. Let me reconsider. We have:

$$\text{E}[X_t | X_{t-1} = r] \le r \left(1 - \frac{r}{n}\right) = r - \frac{r^2}{n}$$

But we also need to account for the multiplicative decrease. Actually, a cleaner analysis uses:
...
Actually, a more direct approach: The problem hint says to use equation (C.25), which is the formula for expectation. Let me use the provided approach:

After $$t$$ iterations, the expected distance is at most:
