...
Consider a tree with $$n$$ nodes. It has exactly $$n - 1$$ edges (since it's a tree). During the entire execution:
...
Let's count edge traversals more carefully. For each node $$x$$ visited during the sequence of successor calls:
