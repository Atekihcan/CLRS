...
{% capture note %}
Counting sort runs in $$\Theta(n + k)$$ time, where $$k$$ is the range of input values. When $$k = O(n)$$, this is linear, beating the $$\Omega(n \lg n)$$ lower bound for comparison sorts. The tradeoff is that it only works on integer-valued keys within a known range.
{% endcapture %}
{% include aside.html title='When Is This Practical?' %}
