...
The algorithm exploits the fact that if we want the $$i$$th smallest element and $$i$$ is small, we don't need to examine all $$n$$ elements carefully. We can eliminate roughly half the elements with a simple pairwise comparison phase.
...
4. Let $$x$$ be the element found. We know $$x$$ is the $$i$$th smallest in $$S$$, but we need to verify it's the $$i$$th smallest overall. The elements that might be smaller than $$x$$ are: (a) the $$i-1$$ elements in $$S$$ smaller than $$x$$, and (b) the larger elements from pairs where the smaller element was $$\le x$$.
