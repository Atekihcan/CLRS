...
{% capture note %}
This insight has practical implications: many real-world sorting tasks involve nearly sorted data (database maintenance, merging update logs, etc.). For such applications, adaptive algorithms like insertion sort or [Timsort](https://en.wikipedia.org/wiki/Timsort){:target="_blank"} (which uses insertion sort for small runs) often outperform quicksort despite having worse worst-case bounds.
{% endcapture %}
