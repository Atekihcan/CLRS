...
Wait, this doesn't give us a counterexample since $$A$$ and $$C$$ are empty. Let me try a slightly larger tree:
...
Now let's check Professor Bunyan's claim with $$a = 3$$, $$b = 7$$, $$c = 15$$:
...
Actually, here's the counterexample:
...
This is the smallest counterexample. The claim fails because node 10 is on the search path, and node 7 is to the right of the search path (it's the right child of node 5, which is on the path), but $$10 > 7$$.
