...
A simpler approach assigns each element a unique tiebreaker that is used only when priorities collide:
...
However, this method is less robust because it does not fully randomize ties when there are more than two elements with the same priority (for example, if three elements all have the same priority, their relative order is still determined by their original indices).
