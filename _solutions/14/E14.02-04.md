...
Consider the nodes visited but not output. These nodes fall into three categories:

1. **Nodes on the path from the root to the smallest key $$\geq a$$**: This is a path of length $$O(\lg n)$$
2. **Nodes on the path from the root to the largest key $$\leq b$$**: Another path of length $$O(\lg n)$$
3. **Ancestors of nodes in the output range**: Every output node has $$O(\lg n)$$ ancestors

Wait, this analysis would give $$O(m \lg n)$$, which is too much. Let me reconsider.

Actually, we can think of it differently. The nodes visited but not output form at most $$O(\lg n)$$ nodes. Here's why:
