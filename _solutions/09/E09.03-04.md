...
Consider any comparison-based algorithm that correctly identifies the $$i$$th smallest element $$x$$. We'll show that the comparison history alone is sufficient to identify which elements are smaller than $$x$$ and which are larger.
...
Therefore, from the comparison graph, we can determine for each element whether it's smaller or larger than $$x$$ by checking reachability:

- An element $$y$$ is smaller than $$x$$ if there's a directed path from $$y$$ to $$x$$
- An element $$y$$ is larger than $$x$$ if there's a directed path from $$x$$ to $$y$$
