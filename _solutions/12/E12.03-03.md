...
Let's analyze each phase separately.
...
Tree sort has the same asymptotic behavior as quicksort: good on random data, poor on sorted data. In fact, there's a deep connection between tree sort and quicksort (see Problem 12-3).
