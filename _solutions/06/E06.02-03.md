...
{% capture note %}
This is the base case for $$\textsc{Max-Heapify}$$'s recursion. When the heap property is already satisfied, the algorithm does no work beyond the initial comparisons. This efficiency is important when $$\textsc{Max-Heapify}$$ is called on nodes that are already in their correct positions, such as during the later stages of $$\textsc{Build-Max-Heap}$$.
{% endcapture %}
{% include aside.html title='Efficient Base Case' %}
