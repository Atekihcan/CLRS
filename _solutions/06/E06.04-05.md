...
description: "Show that when all elements are distinct, the best-case running time of HEAPSORT is Ω(n lg n)."
---
...
{% capture note %}
This result shows that heapsort has a tight $$\Theta(n \lg n)$$ bound in all cases when elements are distinct. There is no input configuration that allows heapsort to run faster than $$\Omega(n \lg n)$$. This is in stark contrast to algorithms like quicksort (which has $$\Theta(n^2)$$ worst case but $$\Theta(n \lg n)$$ average case) or insertion sort (which has $$\Theta(n)$$ best case but $$\Theta(n^2)$$ worst case).

The uniformity of heapsort's performance can be viewed as both a strength and a weakness: it provides predictable performance regardless of input, but it cannot take advantage of favorable input configurations.
{% endcapture %}
