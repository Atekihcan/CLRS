...
Since $$y$$ is the parent of leaf $$x$$, we know that $$x$$ is either the left child or right child of $$y$$. We will show that in each case, $$y$$ is either the successor or predecessor of $$x$$.
...
This makes intuitive sense: a leaf has no descendants, so its successor/predecessor must be an ancestor. Since it is a leaf, the closest ancestor (its parent) is the successor or predecessor.
