...
{% capture note %}
While a sorted array is always a valid min-heap, it's not a very "efficient" heap structure. The heap operations like $$\textsc{Extract-Min}$$ would still work correctly, but a sorted array doesn't take full advantage of the heap's flexibility. Interestingly, a sorted array in increasing order is NOT a max-heap (except for trivial cases), since parents would be smaller than their children.
{% endcapture %}
{% include aside.html title='Sorted Arrays and Heap Efficiency' %}
