...
{% capture note %}
With $$n$$ elements uniformly distributed in $$[0, 1)$$ and $$n$$ buckets, each bucket gets roughly 1 element on average. This makes the insertion sort within each bucket $$O(1)$$ expected time, giving $$\Theta(n)$$ total. The uniform distribution is what makes bucket sort shine.
{% endcapture %}
{% include aside.html title='Why Uniform Distribution Matters' %}
