...
{% capture note %}
The condition $$i \leq A.\textit{heap-size}/2$$ serves as a safeguard to prevent accessing non-existent children. However, in practice, the `if largest == i` check will cause the procedure to return before $$i$$ exceeds this bound, making the while condition redundant in correct executions. Some implementations use `while true` with only the `if largest == i` check for early exit.
{% endcapture %}
{% include aside.html title='Loop Termination Conditions' %}
